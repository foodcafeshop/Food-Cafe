...
import { Utensils, ShoppingBag, Home } from "lucide-react";
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white border-t shadow-lg safe-area-pb">
            <div className="flex items-stretch justify-around px-2">
                <Link
                    href={`/${slug}`}
                    className={`relative flex flex-col items-center gap-1 py-2 px-4 transition-all ${activePage === 'home' ? 'scale-105' : ''}`}
                >
                    {activePage === 'home' && (
                ...
                <Link
                    href={`/${slug}/menu`}
                    className={`relative flex flex-col items-center gap-1 py-2 px-4 transition-all ${activePage === 'menu' ? 'scale-105' : ''}`}
                >
                    {activePage === 'menu' && (
                        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-1 bg-orange-500 rounded-b-full" />
                    )}
                ...
                <Link
                    href={`/${slug}/cart`}
                    className={`relative flex flex-col items-center gap-1 py-2 px-4 transition-all ${activePage === 'cart' ? 'scale-105' : ''}`}
                >
                    {activePage === 'cart' && (
